...
class Pinduoduo_Spider(object):
    ...
    def start(self, id):
        self.change_ua()
        url = "http://yangkeduo.com/goods.html?goods_id=%s" % id
        print url
        res = self.srequest.get(url, headers=self.headers, proxies=self.proxy).text
        html = etree.HTML(res)
...
if __name__ == '__main__':
    url = "http://yangkeduo.com/goods.html?goods_id=9193910"
    category_name = "文件夹"
    dir_name = "商品"
    ps = Pinduoduo_Spider()
